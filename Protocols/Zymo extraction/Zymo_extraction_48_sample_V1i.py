...
def run(polar_express_na_extraction):
	############# Enter number of samples to process.
	...
	def mix_well(instrument, volume, reps):
		for sample_column, store_column in zip (even_wells, odd_wells):
			...
			for i in range(reps):
				...
				instrument.dispense(volume*(6/10), mag_plate[sample_column].bottom(aspirate_depth).move(types.Point(x=-1.25, y=0, z=5)))
			instrument.flow_rate.dispense = 50
			instrument.flow_rate.aspirate = 50
	...
	def mix_well_first(instrument, volume, reps):
		for sample_column, store_column in zip (even_wells, odd_wells):
			...
			instrument.flow_rate.aspirate = 300
			for i in range(reps):
				instrument.aspirate(volume*(8/10), mag_plate[sample_column].bottom(aspirate_depth).move(types.Point(x=0, y=0, z=2)))
				instrument.dispense(volume*(8/10), mag_plate[sample_column].bottom(aspirate_depth).move(types.Point(x=-0, y=0, z=5)))
			instrument.flow_rate.dispense = 50
			instrument.flow_rate.aspirate = 50
			instrument.aspirate(volume*(8/10), mag_plate[sample_column].bottom(2))
			instrument.dispense(volume*(8/10), mag_plate[sample_column].bottom(dispense_depth))
	...
	def remove(instrument, volume):
		for sample_column, store_column in zip (even_wells, odd_wells):
			...
			instrument.pick_up_tip(dirty_rack[store_column])
			instrument.aspirate(volume, mag_plate[sample_column].bottom(aspirate_depth).move(types.Point(x=1, y=0, z=1.5)))
			polar_express_na_extraction.delay(seconds = 5)
			instrument.flow_rate.aspirate = 25
			instrument.aspirate((200-volume), mag_plate[sample_column].bottom(aspirate_depth).move(types.Point(x=1, y=0, z=0)))
			instrument.drop_tip()

	def elute(instrument=p20, volume=10, reps=10, reagent=nf_water):
		magdeck.disengage()
		for sample_column in even_wells:
			instrument.flow_rate.dispense = 50
			instrument.pick_up_tip()
			instrument.aspirate(volume, reagent.bottom(aspirate_depth))
			instrument.dispense(volume, mag_plate[sample_column].bottom(zero).move(types.Point(x=-2, y=0, z=7.5)))
			for i in range(reps):
	...
	def trash_dirty_tips(instrument):
		for store_column in odd_wells:
			instrument.pick_up_tip(dirty_rack[store_column])
			instrument.drop_tip()
	...
	def mix_stock(instrument=p300, reps=15, reagent=nna_lysis_buffer_beads):
		...
		instrument.flow_rate.dispense = 300
		for i in range(reps):
			instrument.aspirate(200, reagent.bottom(1))
			instrument.dispense(200, reagent.bottom(6))
		instrument.flow_rate.aspirate = 50
		instrument.flow_rate.dispense = 50
		instrument.aspirate(200, reagent.bottom(2))
		instrument.dispense(200, reagent.bottom(2))
	...
	# Wait n minutes to excess ethanol to evaporate off
